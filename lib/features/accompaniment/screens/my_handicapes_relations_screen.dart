...
/// Écran « Mes handicapés » (relations accompagnant–handicapé).
/// Liste les liaisons avec statut, permet d'ajouter par ID, accepter ou supprimer.
class MyHandicapesRelationsScreen extends ConsumerStatefulWidget {
  const MyHandicapesRelationsScreen({super.key});

  @override
  ConsumerState<MyHandicapesRelationsScreen> createState() =>
      _MyHandicapesRelationsScreenState();
}

class _MyHandicapesRelationsScreenState
    extends ConsumerState<MyHandicapesRelationsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final list = await repo.getMyHandicapes(acceptedOnly: false);
  ...
  void _showAddDialog(BuildContext context) {
    ...
    showDialog<void>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: Text(strings.addHandicapeById),
        content: TextField(
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () async {
              ...
              try {
                final repo = ref.read(relationsRepositoryProvider);
                await repo.create(handicapId: id);
                await _load();
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
            ...
            child: Text(strings.addHandicape),
          ),
        ],
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(strings.myHandicapes),
        actions: [
          IconButton(
              ...
              : _relations.isEmpty
                  ? Center(
                      child: Column(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          Icon(Icons.accessible_forward, size: 64, color: theme.colorScheme.primary),
                          const SizedBox(height: 16),
                          Text(
                            strings.noHandicapesYet,
                          ...
                          FilledButton.icon(
                            ...
                            label: Text(strings.addHandicape),
                          ),
                        ],
                      ),
                    )
                        ...
                        itemBuilder: (_, i) {
                          ...
                          return Card(
                            child: ListTile(
                              ...
                              title: Text(u?.displayName ?? r.handicapId),
                              subtitle: Column(
