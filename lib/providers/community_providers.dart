...
final locationsProvider = FutureProvider<List<LocationModel>>((ref) async {
  ...
  try {
    ...
    // Backend indisponible → repli catalogue embarqué.
  }
  final places = await AccessiblePlace.loadFromAsset('assets/places.json');
  return places.map((p) => p.toLocationModel()).toList();
});
